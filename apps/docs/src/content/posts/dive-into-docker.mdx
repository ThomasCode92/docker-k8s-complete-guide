...
import running_container from "./diagrams/running-container.excalidraw.png";
...
When trying to run a Container from an Image, the _Docker Client_ reaches out to
...
### How is Docker Running Containers?

<Image src={running_container} alt="running a container" class="mx-auto" />

The Linux Kernel has 2 features that helps running containers:

- **Namespacing**: Isolating resources per process (or group of processes).
- **Control Groups (cgroups)**: Limit amount of resources used per process.

These features are not available on Windows, but Docker has a solution for that.
It uses a _Linux Virtual Machine_, this VM is installed on your Windows or MacOS
machine when you install Docker.
